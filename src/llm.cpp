...
// Llm start
std::string Llm::dump_config() { return config_->config_.dump(); }
...
void Llm::init_runtime() { runtime_manager_.reset(new RuntimeManager()); }
...
nncase::tensor Llm::forward(const std::vector<int> &input_ids) {
    ...
    auto input_ids_tensor =
        _Input<int64_t>(std::vector<int>{seq_len}, runtime_manager_);
    {
        auto input_ids_buffer =
            input_ids_tensor->buffer().as_host().unwrap_or_throw();
        auto input_ids_mapped =
            input_ids_buffer.map(nncase::runtime::map_write).unwrap_or_throw();
        auto ptr =
            nncase::runtime::as_span<int64_t>(input_ids_mapped.buffer()).data();
        for (int i = 0; i < seq_len; i++) {
            ptr[i] = input_ids[i];
        }
        input_ids_buffer.sync(nncase::runtime::sync_write_back, true)
            .unwrap_or_throw();
    }

    auto logits = module_->onForward(input_ids_tensor);
...
int Llm::sample(nncase::tensor &logits, const std::vector<int> &pre_ids) {
    ...
    auto logits_mapped =
        logits_buffer.map(nncase::runtime::map_read).unwrap_or_throw();
...
static std::string apply_template(std::string prompt_template,
...
std::string
Llm::apply_chat_template(const std::vector<PromptItem> &chat_prompts) const {
    ...
    for (; iter != chat_prompts.end() - 1; ++iter) {
        prompt_result +=
            apply_template(chat_template, iter->second, iter->first);
    }
    if (iter->first == "user") {
...
void Llm::chat() {
    ...
    while (true) {
        ...
        history.push_back(
            std::make_pair("system", "You are a helpful assistant."));
        std::cout << "\nQ: ";
        std::string user_str;
...
std::string Llm::generate(const std::vector<int> &input_ids, std::ostream *os,
                          const char *end_with) {
    prompt_len_ = static_cast<int>(input_ids.size());
    history_ids_.insert(history_ids_.end(), input_ids.begin(),
                        input_ids.end()); // push to history_ids_
...
std::vector<float> Llm::softmax(const std::vector<float> &logits) {
    ...
    float max_logit =
        *std::max_element(logits.begin(), logits.end()); // 防止数值溢出
    float sum_exp = 0.0f;
...
float Llm::generate(const std::vector<int> &input_ids,
    ...
    // "tmp/logits_%08lu.bin", idx); std::ofstream ofs(file_name, std::ios::out
    // | std::ios::binary); ofs.write(reinterpret_cast<const char
    // *>(&scores[0]), size * sizeof(float)); ofs.close();

    // dump_memory("dump logits", reinterpret_cast<const float *>(&scores[0]),
    // 128);
...
std::string Llm::response(const std::string &user_content, std::ostream *os,
                          const char *end_with) {
    generate_init();
    if (!end_with) {
        end_with = "\n";
...
std::string Llm::response(const std::vector<PromptItem> &chat_prompts,
    ...
    auto input_ids = tokenizer_->encode(prompt);
    // printf("input_ids (%lu): ", input_ids.size()); for (auto id : input_ids)
    // printf("%d, ", id); printf("\n");
    return generate(input_ids, os, end_with);
}
...
float Llm::response(const std::string &input_id_file,
                    const std::string &target_id_file) {
    generate_init();
    std::vector<int> input_ids;
...
nncase::value_t Llm::embedding(const std::vector<int> &input_ids) {
    ...
    auto inputs_embeds =
        _Input<float>({seq_len, 1, hidden_size}, runtime_manager_);
    auto inputs_embeds_buffer =
        inputs_embeds->buffer().as_host().unwrap_or_throw();
    {
        auto inputs_embeds_mapped =
            inputs_embeds_buffer.map(nncase::runtime::map_write)
                .unwrap_or_throw();
        auto inputs_embeds_ptr =
            nncase::runtime::as_span<int16_t>(inputs_embeds_mapped.buffer())
                .data();
    ...
    inputs_embeds_buffer.sync(nncase::runtime::sync_write_back, true)
        .unwrap_or_throw();
...
std::string Llm::decode(int id) {
    ...
    if (word.length() == 6 && word[0] == '<' &&
        word[word.length() - 1] == '>' && word[1] == '0' && word[2] == 'x') {
...
nncase::value_t Llm::gen_attention_mask(int seq_len) {
    ...
    if (config_->attention_mask() == "float") {
        auto attention_mask =
            _Input<float>({1, 1, seq_len, kv_seq_len}, runtime_manager_);
        auto attention_mask_buffer =
            attention_mask->buffer().as_host().unwrap_or_throw();
        {
            auto attention_mask_mapped =
                attention_mask_buffer.map(nncase::runtime::map_write)
                    .unwrap_or_throw();
            auto ptr =
                nncase::runtime::as_span<float>(attention_mask_mapped.buffer())
                    .data();
            for (int i = 0; i < seq_len; i++) {
        ...
        attention_mask_buffer.sync(nncase::runtime::sync_write_back, true)
            .unwrap_or_throw();
        return attention_mask;
    } else {
        auto attention_mask =
            _Input<int>({1, 1, seq_len, kv_seq_len}, runtime_manager_);
        auto attention_mask_buffer =
            attention_mask->buffer().as_host().unwrap_or_throw();
        {
            auto attention_mask_mapped =
                attention_mask_buffer.map(nncase::runtime::map_write)
                    .unwrap_or_throw();
            auto ptr =
                nncase::runtime::as_span<int>(attention_mask_mapped.buffer())
                    .data();
...
nncase::value_t Llm::gen_position_ids(int seq_len) {
    if (config_->attention_mask() == "glm") {
        ...
        auto position_ids_buffer =
            position_ids->buffer().as_host().unwrap_or_throw();
        {
            auto position_ids_mapped =
                position_ids_buffer.map(nncase::runtime::map_write)
                    .unwrap_or_throw();
            auto ptr =
                nncase::runtime::as_span<int>(position_ids_mapped.buffer())
                    .data();
            if (seq_len == 1) {
        ...
        position_ids_buffer.sync(nncase::runtime::sync_write_back, true)
            .unwrap_or_throw();
        return position_ids;
    } else {
        bool is_glm2 = config_->attention_mask() == "glm2";
...
bool Llm::is_stop(int token_id) { return tokenizer_->is_stop(token_id); }
